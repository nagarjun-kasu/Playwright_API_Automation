...
import patchRequest from "../../testdata/update_partial_booking_payload.json" with { type: "json" };

const BASE_URL = "https://restful-booker.herokuapp.com";

test('Delete Booking details using DELETE request', async ({request}) => {
    ...
    const tokenAPIResponse = await request.post(`${BASE_URL}/auth`, {
      ...
      const patchAPIResponse = await request.patch(`${BASE_URL}/booking/${bId}`, {
        headers: {
      ...
      console.log(" *** Delete Booking Details ***");
      const deleteAPIResponse = await request.delete(`${BASE_URL}/booking/${bId}`, {
        headers: {
          "Content-Type": "application/json",
          "Cookie": `token=${tokenNo}`,
        }
